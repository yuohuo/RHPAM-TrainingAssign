...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "poc-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name poc-service apps/poc-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app poc-service:1.0-SNAPSHOT
		call oc expose svc/poc-service
	) else (
